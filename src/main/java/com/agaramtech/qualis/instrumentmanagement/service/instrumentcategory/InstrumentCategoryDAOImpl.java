...
public class InstrumentCategoryDAOImpl implements InstrumentCategoryDAO {
	...
	public ResponseEntity<Object> getInstrumentCategory(final UserInfo userInfo) throws Exception {
		...
		final String sQuery = "select ic.ninstrumentcatcode,ic.sinstrumentcatname,"
				...
				+ "ic.sdescription ,ic.nrequiredstorage,coalesce(t4.jsondata->'stransdisplaystatus'->>'"
				+ userInfo.getSlanguagetypecode() + "',"
				+ "t4.jsondata->'stransdisplaystatus'->>'en-US') as srequiredstorage "
				+ "from instrumentcategory ic,transactionstatus t1,transactionstatus t2,transactionstatus t3,transactionstatus t4 "
				+ "where  t4.ntranscode = ic.nrequiredstorage "
				+ "and ic.ndefaultstatus=t3.ntranscode  and t1.ntranscode=ic.ncategorybasedflow "
				...
				+ Enumeration.TransactionStatus.ACTIVE.gettransactionstatus() + " and t4.nstatus="
	...
	public ResponseEntity<Object> createInstrumentCategory(final InstrumentCategory instrumentCategory,
		...
		if (instrumentCategoryListByName == null) {
			...
			nsequenceno++;
			//ATE234 Janakumar BGSI-10->Storage Structure ? Instrument id & Site combo load based on Setting table.
			final String insertquery = "Insert into instrumentcategory (ninstrumentcatcode,ncalibrationreq,ncategorybasedflow,sinstrumentcatname,sdescription,ndefaultstatus,dmodifieddate,nsitecode,nstatus,nrequiredstorage)"
					...
					+ "'," + userInfo.getNmastersitecode() + ","
					+ Enumeration.TransactionStatus.ACTIVE.gettransactionstatus() + ","+instrumentCategory.getNrequiredstorage()+")";
		...
		if (objinstrumentCategory == null) {
			...
			if (instrumentCategoryListByName == null) {
				...
				final String updateQueryString = "update instrumentcategory set sinstrumentcatname=N'"
						...
						+ dateUtilityFunction.getCurrentDateTime(userInfo) + "',nrequiredstorage="+instrumentCategory.getNrequiredstorage()+"  where ninstrumentcatcode ="
						+ instrumentCategory.getNinstrumentcatcode();
